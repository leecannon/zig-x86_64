...
pub const VirtAddr = packed struct {
    ...
    /// use `new` or `try_new`.
    pub inline fn new_truncate(addr: u64) VirtAddr {
    ...
    pub inline fn from_ptr(ptr: anytype) VirtAddr {
        ...
        return VirtAddr.new(@ptrToInt(ptr));
    }
    ...
    pub inline fn page_offset(self: VirtAddr) structures.paging.PageOffset {
        return structures.paging.PageOffset.new_truncate(@intCast(u16, self.value));
    }
    
    /// Returns the 9-bit level 1 page table index.
    pub inline fn p1_index(self: VirtAddr) structures.paging.PageTableIndex {
        return structures.paging.PageTableIndex.new_truncate(@intCast(u16, self.value >> 12));
    }
    
    /// Returns the 9-bit level 2 page table index.
    pub inline fn p2_index(self: VirtAddr) structures.paging.PageTableIndex {
        return structures.paging.PageTableIndex.new_truncate(@intCast(u16, self.value >> 12 >> 9));
    }
    
    /// Returns the 9-bit level 3 page table index.
    pub inline fn p3_index(self: VirtAddr) structures.paging.PageTableIndex {
        return structures.paging.PageTableIndex.new_truncate(@intCast(u16, self.value >> 12 >> 9 >> 9));
    }
    
    /// Returns the 9-bit level 4 page table index.
    pub inline fn p4_index(self: VirtAddr) structures.paging.PageTableIndex {
        return structures.paging.PageTableIndex.new_truncate(@intCast(u16, self.value >> 12 >> 9 >> 9 >> 9));
    }
...
pub const PhysAddr = packed struct {
    ...
    pub inline fn new_truncate(addr: u64) PhysAddr {
